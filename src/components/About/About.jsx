import './About.css';

export const AboutMe = () => (
  <section id="about" className="about-section">
      <p>Hello! I’m a developer with 3+ years of&nbsp;experience building responsive and&nbsp;interactive web interfaces with SvelteKit,&nbsp;React, and TypeScript.
        </p><p>
        I enjoy creating maintainable component-based solutions, improving performance, and&nbsp;transforming designs into accessible, user-friendly products.
      </p><p>
      Feel free to get in touch, I’d love to discuss interesting projects and opportunities!
     
{/* 
      My background includes strong experience with semantic markup and styling, combined with JavaScript-driven interaction, performance optimization, and component-based development. <br/>
      I pay close attention to performance and usability, including optimizing Core Web Vitals, and collaborate closely with the team to ensure the final product aligns with both user needs and product goals.<br/>
      It is important for me to create interfaces that not&nbsp;only look good, but are also accessible, well-structured and&nbsp;easy to&nbsp;maintain.  */}
      </p>
    
      <p className="location">Based in Amsterdam, NL.</p>
  </section>
);
